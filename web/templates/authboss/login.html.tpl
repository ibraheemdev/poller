...
<form action="{{mountpathed "login"}}" method="POST">
    ...
    {{with .modules}}
        {{with .remember}}
            <input type="checkbox" name="rm" value="true"> Remember Me</input><br/>{{end}}
    {{end -}}
    {{with .redir}}<input type="hidden" name="redir" value="{{.}}"/>{{end}}
    <button type="submit">Login</button>
    {{with .modules}}
        {{with .recover}}<br/>
            <a href="{{mountpathed "recover"}}">Recover Account</a>
        {{end}}
    {{end -}}
    {{with .modules}}
        {{with .register}}<br/>
            <a href="{{mountpathed "register"}}">Register Account</a>
        {{end}}
    {{end -}}
</form>
{{end}}
